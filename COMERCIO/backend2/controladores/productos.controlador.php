			...
			if(preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $datos["tituloProducto"]) && preg_match('/^[,\\.\\a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["descripcionProducto"]) ){
				...
				if(isset($datos["fotoPrincipal"]["tmp_name"]) && !empty($datos["fotoPrincipal"]["tmp_name"])){
					...
					if($datos["fotoPrincipal"]["type"] == "image/png"){
						...
						if($datos["fotoPrincipal"]["type"] == "image/gif"){
						...
						$rutaFotoPrincipal = "../vistas/img/productos/".$datos["rutaProducto"].".gif";

						$origen = imagecreatefrompng($datos["fotoPrincipal"]["tmp_name"]);						

						$destino = imagecreatetruecolor($nuevoAncho, $nuevoAlto);
						imagefilledrectangle($destino, 0, 0, 99, 99, 0xFFFFFF);

						imagestring($destino, 3, 40, 20, 'Biblioteca GD', 0xFFBA00);
...
imagegif($destino);
						...
						imagegif($destino, $rutaFotoPrincipal);
...
imagedestroy($destino);
				...
				/*=============================================
				VALIDAR IMAGEN PRINCIPAL DOS
				=============================================*/

				$rutaFotoPrincipal2 = "../vistas/img/Portada2/default/default.jpg";

				if(isset($datos["fotoPrincipal2"]["tmp_name"]) && !empty($datos["fotoPrincipal2"]["tmp_name"])){

					/*=============================================
					DEFINIMOS LAS MEDIDAS
					=============================================*/

					list($ancho, $alto) = getimagesize($datos["fotoPrincipal2"]["tmp_name"]);	
					...
					if($datos["fotoPrincipal2"]["type"] == "image/jpeg"){
						...
						$rutaFotoPrincipal2 = "../vistas/img/Portada2/".$datos["rutaProducto"].".jpg";

						$origen = imagecreatefromjpeg($datos["fotoPrincipal2"]["tmp_name"]);						

						$destino = imagecreatetruecolor($nuevoAncho, $nuevoAlto);

						imagecopyresized($destino, $origen, 0, 0, 0, 0, $nuevoAncho, $nuevoAlto, $ancho, $alto);

						imagejpeg($destino, $rutaFotoPrincipal2);

					}

					if($datos["fotoPrincipal2"]["type"] == "image/png"){
						...
						$rutaFotoPrincipal2 = "../vistas/img/Portada2/".$datos["rutaProducto"].".png";

						$origen = imagecreatefrompng($datos["fotoPrincipal2"]["tmp_name"]);						
						...
						imagepng($destino, $rutaFotoPrincipal2);
				...
				if($datos["selActivarOferta"] == "oferta"){
					...
					$datosProducto = array(
						   ...
						   "imgFotoPrincipal2"=>substr($rutaFotoPrincipal2,3),
						   "oferta"=>1,
		...
		if(isset($datos["idProducto"])){
			...
			if(preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $datos["tituloProducto"])  && preg_match('/^[,\\.\\a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["descripcionProducto"]) ){
				...
				if(isset($datos["fotoPrincipal"]["tmp_name"]) && !empty($datos["fotoPrincipal"]["tmp_name"])){
                    ...
                    imagedestroy($destino, $rutaFotoPrincipal);
...
/*=============================================
				VALIDAR IMAGEN PRINCIPAL 2
				=============================================*/

				$rutaFotoPrincipal2 = "../".$datos["antiguaFotoPrincipal2"];
				...
				if(isset($datos["fotoPrincipal2"]["tmp_name"]) && !empty($datos["fotoPrincipal2"]["tmp_name"])){
					...
					unlink("../".$datos["antiguaFotoPrincipal2"]);
	...
	static public function ctrEliminarProducto(){
		...
		if(isset($_GET["idProducto"])){
			...
			if($_GET["imgPrincipal"] != "" && $_GET["imgPrincipal"] != "vistas/img/productos/default/default.jpg"){
				...
				if($_GET["imgPrincipal2"] != "" && $_GET["imgPrincipal2"] != "vistas/img/Portada2/default/default.jpg"){

				unlink($_GET["imgPrincipal2"]);		
